...
export type Database = {
  ...
  public: {
    Tables: {
      ...
      evo_api_keys: {
        // EVO-6 (migration 0082) — partner credentials for /api/evo/v1/chat.
        // The RAW key is shown ONCE at creation and NEVER stored: key_hash
        // (sha256 hex) is the lookup identity; key_prefix (first 12 chars)
        // is display-only. Zero client policies by design (0080 posture —
        // here it is a SECRET-SAFETY requirement: a client-readable key
        // table would let any user mint/read partner keys). Service-role
        // only: partner chat route (hash lookup) + admin partners route.
        Row: {
          id: string;
          partner_name: string;
          key_hash: string;
      ...
      evo_api_usage: {
        // EVO-6 (migration 0082) — honest per-request metering ledger:
        // ONE row per partner chat request attempt with its final status
        // (success/rate_limited/quota_exceeded/rejected/error). Quota
        // enforcement counts current-month success rows; the full ledger
        // (including rejections) is the abuse-audit trail. Zero client
        // policies by design (service-role only).
        Row: {
          id: string;
          created_at: string;
          api_key_id: string;
        ...
        Relationships: [
          {
            foreignKeyName: "evo_api_usage_api_key_id_fkey";
            columns: ["api_key_id"];
            isOneToOne: false;
            referencedRelation: "evo_api_keys";
