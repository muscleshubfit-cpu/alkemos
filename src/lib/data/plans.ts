...
import type { Database, Plan, SupportTicket } from "@/lib/supabase/types";
import type { MembershipTier } from "@/lib/memberships";
// Type-only imports — erased at build, so the client bundle never pulls in
// the generator or its AI dependencies (same pattern as blog.ts BlogFaq).
import type { NutritionPlanContent, WorkoutPlanContent } from "../plan-generator";

/** Parsed plan.content shape as generated by plan-generator.ts. */
export type PlanContent = NutritionPlanContent | WorkoutPlanContent;

/** Insert shape of the plans table (single source: generated supabase types). */
export type PlanInsert = Database["public"]["Tables"]["plans"]["Insert"];

/** Update shape of the plans table. */
export type PlanUpdate = Database["public"]["Tables"]["plans"]["Update"];

export async function listPlans(clientId: string): Promise<Plan[]> {
 ...
 return read<Plan[]>(LS_PLANS, []).filter((p) => p.client_id === clientId);
}

/** Coach sees ALL plans including drafts (for review/approval). */
export async function listAllClientPlans(clientId: string): Promise<Plan[]> {
 if (isSupabaseConfigured && supabase) {
 const { data } = await supabase
 .from("plans")
 .select("*")
...
export async function getSwapUsage(userId: string) {
 ...
 const tierId = (userSub?.tier ?? "free") as MembershipTier;
...
export async function addPlan(plan: PlanInsert): Promise<Plan> {
 if (isSupabaseConfigured && supabase) {
...
export async function updatePlan(id: string, updates: { title?: string; notes?: string; content?: PlanContent | null; status?: string; is_current?: boolean }): Promise<Plan | undefined> {
 if (isSupabaseConfigured && supabase) {
 ...
 // Honest boundary cast: PlanContent (typed alias) → supabase Json.
 payload.content = updates.content as PlanUpdate["content"];
 }
 const { data, error } = await supabase
 .from("plans")
 .update(payload)
 ...
 if (idx >= 0) {
 const merged: Plan = { ...all[idx], ...updates };
 if (updates.content !== undefined) {
 merged.content = updates.content as Plan["content"];
 }
 all[idx] = merged;
 write(LS_PLANS, all);
 }
