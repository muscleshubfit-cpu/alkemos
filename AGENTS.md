...
- **EVO-6 PARTNER API LAW (2026-09-10, owner order «ابدأ evo 6»):** the public partner surface `/api/evo/v1/chat` + `/embed/evo.js` + `/embed/widget` is gated by `EVO_PARTNER_API_ENABLED` (404 when off) and serves GENERAL CONVERSATION ONLY — plan/swap intents are rejected 400, no subscriber context/memory/plan generation ever reaches a partner request, `detectEvoCrisis` guards every message. Keys (`evo_api_keys`, 0082) are HASH-ONLY (sha256 — the raw key is shown ONCE at creation via `/admin/evo-partners`, rotation = new key + deactivate old; there is NO delete path — «الإيقاف UPDATE لا delete»); quota = current-UTC-month SUCCESS rows of the honest per-attempt ledger `evo_api_usage` (rejections included as the abuse audit); rate limit 30/min per key+IP; the metered success row is written SYNCHRONOUSLY before the response (quota integrity). Themes are WHITELIST-validated (`validatePartnerTheme` — accent #hex · https-only logo · bounded greetings; widget consumes CSS variables, never markup). The platform floating widget NEVER mounts on `/embed/*` (EVO_CHAT SURFACE LAW — one surface per context). Conversation export (W5.6 of 168) is REMOVED by owner order «لا داعى لتصدير المحادثة الغية» — `evo-export.ts` deleted, do not resurrect.
...
- **BRAND NAME LAW:** the site name is written EXACTLY «Alkemos» in every user-visible string (metadata, i18n, legal, landing, affiliate, AI prompts, PayPal descriptions); legacy spellings «Alkemos» / «Alkemos Egypt» / «Alkemos» are FORBIDDEN in new code; lowercase technical identifiers stay as-is.
