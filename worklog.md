...
- **مسح حي كمّي قبل الحكم (P1-1):** كل مقالات السايت ماب (69) — العناوين الخمسة فوق الميزانية كلها لاحقة « — Alkemos» مخزنة في meta_title القديم (نزعها = 60–67 حرفاً داخل ميزانية الـ70 بلا قصّ كلمات) + الرمادية الـ23 كلها كذلك + صفر EN فوق 60 (علاج 181 قائم)
- **(P1-1 أ) استخراج القانون:** كتلة SEO-GEO-6.3/181 انتقلت حرفياً إلى `src/lib/blog-meta-title.ts` (صفرية الاعتمادات — نمط exercises-shared في 186: المولد يحمل سلسلة مزودي AI فلا يُستورد في مسارات العرض) — `blog-pipeline.ts` يعيد التصدير (p5-publish وسكربت علاج 181 والاختبارات القائمة تعمل كما هي)
- **(P1-1 ب) التوصيل (نمط 187 — صفر كتابة DB):** `fetchBlogForOG` (blog-server.ts) يطبّق `clampMetaTitle` على المخزن — نقطة الخنق الوحيدة لـ`<title>` + og:title + twitter:title + headline السكيما + فتات الخبز بالمرآتين + بطاقة `/api/og-image` + عنوان المشاركة في BlogArticlePage (مكون عميل — الوحدة الصفرية تجعله آمن الحجم)
...
- **(P2-1) عبر Cloudflare API بتفويض المالك:** قياس حي: كل الصفحات العامة تصل المتصفح بـ`private, no-cache, no-store` رغم CF HIT (رؤوس next.config headers() تنتصر مع `next start` فقط لا على Vercel الديناميكي) — تحديث قاعدة `alkemos-public-html-cache` (نفس تعبير 149: العام بلا نقاط وباستثناء الخاص) بإضافة `browser_ttl override 300` عبر PUT لنقطة دخول المرحلة (نقطة القاعدة المفردة ترد 405) مع بقاء edge_ttl 3600 — 300 لقانون نضارة 128/136 (نشرات متعددة يومياً وكاش المتصفح لا يُنقّى)
- **التحقق الحي لـP2-1 فوراً:** العام (مقالات EN/AR + الرئيسيتان + about + memberships + تمارين + hubs) = `private, max-age=300, must-revalidate` + CF HIT · الخاص (/auth · /admin · /api/build-info) بلا كاش كما كان · `/_next/static` immutable سنة — (التوجيه الظاهري private أبقته إعادة كتابة CF؛ المتصفحات تحترمه مع max-age — موثق §12.46)
- **الحراس (+10 → 1027/1027):** `blog-meta-title-render.test.ts` — الحوادث الخمس كناريات حرفية (بايت-مطابقة من المسح الحي → الصورة النظيفة الدقيقة، ؟ محفوظة) · رمادية تنزل تحت الميزانية · الوحدة صفرية الاعتمادات · إعادة التصدير لا التعريف · التوصيلان باقيان
- **البوابات:** tsc 0 · eslint 0/0 · vitest 1027/1027 · next build exit 0 · docs_audit (phase=189) · docs_parity · migration_audit (صفر ميجريشنز — إنذار ip_key الكاذب الموثق) · stale-refs · ui-wiring
...
- قانون العنوان صار مطبقاً على ثلاث طبقات بمصدر واحد: المولد (p5) + المعالجة المخزنة (181) + العرض (189) — أي بيانات مستقبلية أو قديمة لا تستطيع تسريب عنوان فوق الميزانية
- كاش المتصفح: أول تخزين محلي للـHTML في تاريخ المنصة (5 دقائق) فوق كاش الحافة (ساعة) — الأسطح الخاصة محمية بلا تغيير
