...
- **كاش الصفحات العامة (SEO-GEO-4، 2026-09-08):** الصفحات العامة ترجع `Cache-Control: public, max-age=0, must-revalidate, s-maxage=3600, stale-while-revalidate=86400` — المتصفح يعيد التحقق في كل زيارة (نفس حداثة no-store السابقة) والحافة تحتفظ بالصفحة ساعةً وتخدم قديمة أثناء التحقق الخلفي (TTFB أسرع + زحف أرخص) — الخاصة/المصادقة/API تبقى no-store — **مصدر الحقيقة الوحيد للقاعدة: `next.config.ts` headers()** (متحقق حيًا أن رأس middleware يتفوق عليه رأس إطار العمل) + كوكي اللغة يُكتب عند التغيّر فقط في `src/middleware.ts` (استجابات بلا Set-Cookie = قابلة للكاش على الحافة).
