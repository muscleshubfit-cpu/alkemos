...
- **EVO Partner API (EVO-6):** public `/api/evo/v1/chat` for partners — bearer API keys (`evo_api_keys`, SHA-256 hashed, monthly quotas), per-request metering ledger (`evo_api_usage`), per-key rate limiting + abuse gates (general conversation only — no plans, no personal context), crisis shield included — behind `EVO_PARTNER_API_ENABLED`
- **Embeddable EVO Widget (EVO-6):** one-line embed `<script src="https://alkemos.com/embed/evo.js" data-key="pk_live_…"></script>` → isolated iframe chat; white-label theming per partner (accent/logo/greeting via CSS variables) managed in `/admin/evo-partners`
