...
### For Users
- **Homepage (restructured 2026-09-15, owner order «من Presentation إلى Product Website»):** ONE narrative path instead of the old section-per-service catalog — Hero (single primary CTA) → Tools & AI (the AI-plan lead card + the EVO warrior card merged in with a benefit line + a compact tools grid) → ONE tabbed Library discovery hub (exercises / programs / foods / blog; only the active tab renders) → Coaching spotlight → Memberships (tiers + the collapsed "Why Alkemos" comparison) → merged closing act (FAQ + final CTA) → footer. For-Coaches (B2B) and Affiliate moved out of the homepage into the footer Partners list + the header Partners group (For-Coaches previously had no persistent access point outside the homepage section); the newsletter form moved into the shared footer (same `/api/tools/lead` pipeline, now on every public page); per-section chrome CTAs demoted to quiet text links and the meander dividers reduced to the two act boundaries. Display-only restructure — no service, route, price, quota, or API changed — where: `src/components/views/LandingView.tsx`, `src/components/SiteFooter.tsx`, `src/components/SiteHeader.tsx`
...
- **جدول المقارنة:** قسم العضويات في الرئيسية يحمل جدول مقارنة سريع القراءة (✅/❌) بين المنصة والبدائل التقليدية (مدرب شخصي تقليدي) والتطبيقات المجانية — يُظهر القيمة المضافة بوضوح.
