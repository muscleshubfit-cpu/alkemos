...
| **V3 GO (owner order 2026-09-23 «اكمل»)** | **V3 executed** (phase 263): **§16.2 hero** (C-9): phones get the `max(56vh, natural)` stage floor (332→473px at 390px) + the CTA pair STACKS full-width with the quiet centered Log in 28px clear of the pair (md+ row unchanged, 20px gap). **§16.3 chips** (C-11): ONE scroll-snap row on touch — nowrap + proximity snap + symmetric 24px edge-fade masks (RTL-safe) + chips `flex-shrink:0; white-space:nowrap` (the «بطن/كور» internal wrap measured and fixed); md+ keeps the centered wrap. **§11 density** (C-10): path cards icon+title one row + 2-line clamp + p-5 (block 818→**547px** grid, section target ≤720 ✓) · plan/memberships/EVO cards p-5 mobile + memberships copy clamped 3 lines · food cards min-h-10 title floor + gap-2 rhythm (C-1-VLM dead space). **§13 CTA standard** (C-15): primary 48px touch/52px md+ · secondary 44/48 (min-height floors, paddings feed the floor only) · carousel arrows 44px touch (V0). **§13.3 O-3 asymmetry** (C-13): memberships card = filled `.btn-chrome`, coaching keeps outline. **§11 unified hover**: `.marble-card.card-lift` — 2px lift + `--shadow-lift` + warm-graphite hairline shift, 200ms, reduced-motion-safe, on all six interactive card families. **§16.1 pacing**: section py-12→py-10 mobile + header-grid gaps mt-10→mt-8 (md+ untouched). Static core page (no dynamic strips) **9,474px EN / 9,257px AR** at 390px (pre-V3 same-condition baseline 9,961/9,561) — the ≤9,500 exit criterion holds for the static page; the live page additionally carries the deferred LEARN carousel + featured-coaches strip (~1,200px, content-bearing, out of V3's touch per §11 «exercise/program cards unchanged» + zero-content-removal law) — measured live post-deploy and documented in the worklog. | Gates: tsc 0 · eslint 0 · vitest 100/**1698** (+7 V3 canaries; V0 touch-target canary re-pinned same-frame to the raised 48/52 standard) · build 2020/2020 · contrast matrix green · parity/migration/stale-refs/ui-wiring ✓. Zero horizontal overflow across **16 contexts** (390/768/1024/1440 × EN/AR × light/dark). CTA audit table exact: mobile 48/44, desktop 52/48. VLM 6 contexts + targeted asymmetry shot all PASS. Next wave V4 (copy K-1..K-7) awaits the owner's order per O-5 discipline. |
| **V4 GO (owner order 2026-09-23 «اكمل V4 الآن… ابدأ مباشرة من K-1..K-7، دون إعادة كتابة البنود السابقة»)** | **V4 executed** (phase 265): §14 copy, surgical only (§23.9 law — no wholesale rewrite). **K-1**: «توليدات خطط أكثر» → «توليد خطط أكثر» ×3 (FAQ ×2 + memberships card) — the mechanical pseudo-plural is dead on the homepage (numeric counts «4/8 توليدات شهريًا» in memberships.ts untouched — pricing-page source, out of scope). **K-2**: coaches H2 «مدربون مميزون على Alkemos» (restores the EN «featured» meaning). **K-3**: newsletter button «اشترك مجانًا» (drops the salesy «الآن»). **K-4**: footer credit «صُنع بعناية لمجتمع اللياقة» (mirrors the EN «care»). **K-5**: core chip «كور» plain (register of بايسبس/ترايسبس; exercises-library category labels keep «بطن/كور» — §19 scopes K-5 to the homepage chip). **K-6**: hero subtitle pair trimmed ~25%, audit-verbatim (EN 17→15 words, AR 13→9 — relieves C-9). **K-7**: memberships card enumeration 4→3 items (EN drops «bigger save capacity with export», AR drops «حفظ وتصدير أوسع»; EN core trio kept verbatim; coaching card untouched at 25/17 words). **§19 tailwind task**: indigo `glow` + amber `gold` boxShadow entries removed from `tailwind.config.ts` — dead-by-construction (Tailwind v4 CSS-first, no `@config` anywhere; live `shadow-glow`/`shadow-gold`/`shadow-card` utilities come from globals.css `@utility` over monochrome tokens); full config-file removal deferred to the V5 sweep (C-18). **O-1b logo regen skipped** (default No). | Exit criteria all green: FAQ JSON-LD **single-source preserved** (`getFAQSchema(faqs)` derives — schema carries the new K-1 phrasing ×2; no hardcoded schema in the view, canary-pinned) · **og:title/description untouched** (verified in served HTML) · **MSA detector clean** (90 marketing-surface tests; «كور» not a dialect marker, «Alkemos/EVO» whitelisted). Canaries: **+8 V4 tests** (K-1..K-7 + JSON-LD single-source; banned old strings + required new) + **2 re-pins** (hero pair, footer tagline) in the SAME commit (§21.3). Gates: tsc 0 · eslint 0 · vitest 100/**1706** · build 2020/2020 · parity/migration/stale-refs/ui-wiring ✓ · contrast matrix green (zero token changes). Page heights **stable**: 9,474px EN / 9,257px AR at 390px (trims are within-line — same line counts) · zero horizontal overflow. VLM: AR+EN light hero, AR dark hero, chips one-row, memberships 3-line clamp, footer — **all PASS** (one VLM misread of the tiny newsletter button «اشترك الآن» refuted by live DOM: «اشترك مجانًا» ✓). Next wave V5 (sweep + DESIGN.md + C-18 dead-CSS + marble-asset deletion in the cache window) awaits the owner's order per O-5 discipline. |
| **V5 GO (owner order 2026-09-23 «اكمل V5 الختامية… بنفس بروتوكول المشروع: audit → execute → test → document → commit → push → verify production. لا تعِد تنفيذ أو شرح أي بند تم حسمه في V1–V4، ولا توسّع النطاق خارج خطة V5 المعتمدة»)** | **V5 executed** (phase 267 — the closing wave): **C-18 dead-generation sweep** in globals.css — the Apple-style `:root` generation's duplicated shadcn mappings (dead values; the Marble & Chrome generation wins the cascade for every shared name) and the `.dark` indigo block (nothing ever set `class="dark"` — the engine is `data-theme`) are deleted; the only live survivors (–radius, –primary(+fg), –destructive/success/warning/gold(+fg), –gradient-primary, –shadow-glow, –shadow-card) consolidated into a clearly-labeled shadcn/ui-compat block, values byte-identical (a sweep, NOT a re-theme — `--primary` stays Apple blue on app/UI surfaces only, pre-existing and out of VRD scope); `--chart-1..5` + @theme mappings deleted (zero consumers). **Dead utilities deleted** (grep-proven, each with the evidence in the worklog): glass, glass-gold, gradient-border, grid-bg, neon-text, text-shimmer, text-gradient-gold, bg-gradient-gold, bg-hero-glow, shadow-gold, shadow-soft-lg, transition-smooth/fast/slow, animate-fade-in/-in-up, gold-pulse/neon-flicker/float-up/pulse-ring/shimmer keyframes, scroll-hidden, skeleton-shimmer, `.hero-seals`, `.cmp-details`, `.tap-target`, `.no-print` + their reduced-motion/print references. **tailwind.config.ts deleted** (dead-by-construction under Tailwind v4 CSS-first — V4's deferral executed). **texture-marble-light/dark.webp deleted** (§19 cache-law window satisfied: V2 retirement shipped through V3+V4 production cycles; zero network requests for them verified locally). **DESIGN.md fully rewritten** to the post-redesign system (binding reference — §2 documents the single-token architecture, the V5 tombstones, the owner-artwork exemption law, and the pixel-equivalence verification standard). | Exit criteria all green: full gate suite (tsc 0 · eslint 0 · vitest 100/1706 · build 2020/2020 · parity/migration/stale-refs/ui-wiring/docs_audit ✓) · contrast matrix green (zero token changes) · **rendering equivalence PROVEN pixel-wise**: the same 10-context matrix (home EN/AR × light/dark × 1440/390 + auth light/dark) captured from the pre-V5 and V5 builds — 3 byte-identical, 7 with zero severe pixels (channel delta >8: none; residual noise = carousel/marquee animation timing only) · secondary-pages visual pass: memberships/tools/blog/evo/auth EN+AR light+dark + mobile, tokens inherited correctly (#FAF8F5/#12100E computed), 95 homepage-linked routes 200 both languages, zero texture-marble requests, zero horizontal overflow · VLM final pass 20/20 PASS (one FAIL verdict on the AR dark memberships banner refuted objectively: the cool-leaning artwork is owner art from Phase 126, unchanged by V5, renders pixel-identical, and the same asset PASSED in the EN shot — VLM judgment variance, documented as informational per the owner-artwork law) · homepage height 9,474px EN at 390px (matches V4 exactly). **The VRD program V0–V5 is COMPLETE.** |
