...
- **الإثبات (Drill #2):** `.github/workflows/storage-restore-drill.yml` (تشغيل يدوي فقط) — استعادة كاملة إلى باكت مؤقتة `restore-drill-<run>` خاصة + قراءة كل كائن راجعًا ومطابقة sha256 حرفيًا + حذف الباكت المؤقتة في نفس التشغيل (`if: always()`). أول تشغيل موثق النتيجة يُسجّل في تقرير دريل مستقل (`docs/RECOVERY-DRILL-2-STORAGE-REPORT-2026-09-20.md`).
