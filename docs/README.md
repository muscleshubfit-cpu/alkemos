...
| `docs/SUPABASE-FULL-RECOVERY-RUNBOOK.md` | Full Supabase-loss recovery runbook (P1-5 + P0-4 + P0-3): two-track schema replay (41 integration-auto + 59 manual in order with supersession flags) → db-restore dry/apply → auth settings table (Google OAuth · Brevo SMTP · autoconfirm+trigger 0074) → storage buckets (3 private + coach-public) → post-restore verification + known unrecoverable limits (passwords · storage files) | LIVE (recovery reference — INDEX.md stays the binding ledger on any conflict) | 2026-09-20 |
